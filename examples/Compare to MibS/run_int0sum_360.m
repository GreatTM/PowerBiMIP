...
mps_file = fullfile(base_dir, 'MibSdata', 'notInterdiction', 'Random', 'int0sum_i0_360.mps');
txt_file = fullfile(base_dir, 'MibSdata', 'notInterdiction', 'Random', 'int0sum_i0_360.txt');
...
ops = BiMIPsettings(...
    ...
    'solver', 'cbc', ...            % Use CBC (or 'gurobi', 'cplex')
    'verbose', 2, ...               % Show detailed output and convergence plot
    'max_iterations', 100, ...      % Set iteration limit (larger instance might need more)
    'optimal_gap', 1e-4 ...         % Convergence tolerance
);

% Note: Large instances might require longer timeout or different solver params
% ops.ops_MP = sdpsettings('solver', 'gurobi', 'verbose', 0); % Example customization
